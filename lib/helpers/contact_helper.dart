...
class ContactHelper{ //usando o padrão de projeto singleton para evitar que o objeto seja instanciado mais de uma vez
  ...
  Future<List>getAllContacts() async{
    Database dbContact = await db;
    List listMap = await dbContact.rawQuery("SELECT * FROM $contactTable");//lista de mapas
    List<Contact> listContact = List();//declarada uma lista de contatos
    for (Map m in listMap ){//pra cada mapa da minha lista de mapas
      listContact.add(Contact.fromMap(m));//transformo em um contato
    }
    return listContact;
  }
